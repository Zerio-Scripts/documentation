...
import { UpdateIcon, DownloadIcon, BackupIcon, AlertTriangleIcon } from "@radix-ui/react-icons";
...
<Tabs items={['Windows', 'Linux', 'txAdmin']}>
  <Tabs.Tab>
    **Windows Server Update:**
    ...
    2. **Download latest version** from [Keymaster](https://keymaster.fivem.net)
    3. **Backup current resource**: Copy `zerio-resourcename` to a backup folder
    4. **Extract new files**: Unzip the downloaded update
    5. **Replace old files**: Delete old resource folder and replace with new one
    6. **Restore configurations**: Copy your custom configuration files back
    7. **Check SQL updates**: Import any new SQL files if provided
    8. **Start server**: Restart your FiveM server
    9. **Test functionality**: Verify everything works correctly
  </Tabs.Tab>
  <Tabs.Tab>
    **Linux Server Update:**
    ...
    cp -r zerio-resourcename/ zerio-resourcename-backup/
    
    # Remove old version
    rm -rf zerio-resourcename/
    
    # Extract new version
    unzip /path/to/downloaded/zerio-resource.zip
    
    # Restore your config files
    cp zerio-resourcename-backup/config.lua zerio-resourcename/
  ...
  <Tabs.Tab>
    **txAdmin Web Panel:**
    ...
    1. **Access txAdmin panel** in your web browser
    2. **Stop the server** using the web interface
    3. **Use Resource Manager** to upload the new version
    4. **Replace existing resource** when prompted
    5. **Import SQL updates** if any are provided
    6. **Start the server** through the web panel
    7. **Monitor live console** for any errors
  </Tabs.Tab>
</Tabs>
